...
def calcular_nulos(dataframe):
    '''Devuelve el porcentaje de valores nulos por columna.'''
    porcentaje_nulos = (dataframe.isnull().sum()/dataframe.shape[0])*100
    return porcentaje_nulos

def analisis_general_cat(dataframe):
    '''Analiza cada columna categórica: muestra distribución, número de valores únicos y estadísticas básicas.'''
...
def analisis_categoricas(dataframe, col_cat):
    '''Grafica columnas categóricas contra churn_label, excluyendo variables no relevantes.'''
    col_excluidas = ["churn_label", "customer_id", "state", "contact_date", "last_transaction_date"]
    ...
    if len(col_cat) == 0:
        ...
        for i, col in enumerate(col_cat):
            ...
            axes[i].set_title(f'Distribución de {col} por churn_label')
            axes[i].set_xlabel(col)
            axes[i].set_ylabel('Frecuencia')
            axes[i].tick_params(axis='x', rotation=90)
        for j in range(i + 1, len(axes)):
